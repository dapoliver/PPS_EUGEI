...
PPS_scored <- PPS_scored %>% mutate(Transition = case_when(day_exit>1095.75 ~ 0,
                                                           TRUE ~ Transition),
                                    day_exit = case_when(day_exit>1095.75 ~ 1096,
                                                         TRUE ~ day_exit))
...
PPS_scored_prog_mat <- model.matrix(~.-1,PPS_scored[,c(3:8,10:14,16:17,24)])
PPS_scored_prog_mat <- as.data.frame(PPS_scored_prog_mat) %>% subset(select=c(-Gender2))
PPS_scored_prog_mat$day_exit <- PPS_scored$day_exit
PPS_scored_prog_mat$Transition <- PPS_scored$Transition
# PPS_scored_prog_mat <- as.data.frame(PPS_scored_prog_mat) %>% subset(select=c(-ZRE_SCZ_imp_EUR_0.1))
PPS_summ <- PPS_scored_prog_mat %>% aggregate(.~Transition, FUN=c("mean"))
PPS_summ_long <- tidyr::pivot_longer(PPS_summ,
                                        cols = -Transition,  # Specify the binary variable
                                        names_to = "variable",         # Name of the new column for variable names
                                        values_to = "value")           # Name of the new column for values
ggplot(data=PPS_summ_long, aes(x=variable, y=value*100, fill=factor(Transition))) +
  geom_col(position = position_dodge(width = 0.9)) +
  geom_text(aes(label=paste(round(value*100),"%", sep=""),y=(value*100)+10), size=5,position = position_dodge(width = 0.9)) +
  scale_x_discrete(labels=c("Adult Life Events","Anhedonia","Cannabis use", "Childhood Trauma", "Ethnicity (Other in high \nethnic density area)","Ethnicity (Other in medium \nethnic density area)",
                            "Ethnicity (Other in low \nethnic density area)","Ethnicity (Black in low \nethnic density area)", "Gender (Male)",
                            "Non-right-handedness", "Non-immigrant","Second generation immigrant", "First generation immigrant", "Parental SMI",
                            "Low Paternal SES", "Pollution", "Tobacco use", "Urbanicity"), name="Factor") +
  scale_y_continuous(name="Percentage (%)") +
  labs(fill = "") +  # Change legend title
  scale_fill_discrete(labels = c("CHR-P+", "CHR-P-")) +  # Change legend labels  theme_classic()+
  coord_flip() +
  theme_classic() +
  theme(axis.text.y = element_text(angle = 35, hjust = 1, size=10))

ggsave("~/Dropbox/Work/PPS/EU-GEI/Plots/variables.png",width = 8, height = 9)

PPS_scored_prog_mat <- as.data.frame(PPS_scored_prog_mat) %>% subset(select=c(-day_exit,-Transition,-Ethnicity6, -Ethnicity3, -Immigration2.5))
y <- Surv(PPS_scored$day_exit, PPS_scored$Transition)
PRS_multi <- nestcv.glmnet(y=y, x=PPS_scored_prog_mat,family="cox",
                           n_outer_folds = 10)
PRS_multi <- glmnet(y=y, x=PPS_scored_prog_mat,family="cox",
                           alpha=0)
x_cox <- coxph(Surv(PPS_scored$day_exit, PPS_scored$Transition) ~ PPS_scored_prog_mat$Gender0 + PPS_scored_prog_mat$Handedness2 + PPS_scored_prog_mat$Urbanicity1 +
        PPS_scored_prog_mat$Pollution2 + PPS_scored_prog_mat$Ethnicity1  + PPS_scored_prog_mat$Ethnicity3 + PPS_scored_prog_mat$Ethnicity3.5 + #PPS_scored_prog_mat$Ethnicity6 +
        PPS_scored_prog_mat$Immigration0 +PPS_scored_prog_mat$Immigration2    + PPS_scored_prog_mat$Paternal_SES1 +  PPS_scored_prog_mat$Parental_SMI5.5 +
        PPS_scored_prog_mat$Adult_Life_Events5.5 + PPS_scored_prog_mat$Tobacco3 +  PPS_scored_prog_mat$Cannabis7 +  PPS_scored_prog_mat$Childhood_Trauma4  + PPS_scored_prog_mat$Anhedonia6.5 + PPS_scored_prog_mat$ZRE_SCZ_imp_EUR_0.1)

cv.PRS_multi <- nestcv.glmnet(y=Surv(PPS_scored$day_exit, PPS_scored$Transition), x=as.data.frame(PPS_scored_prog_mat),family="cox",
                           n_outer_folds = 10, alpha=1) |>
  repeatcv(10, repeat_folds = folds, rep.cores = 2)

summary(cv.PRS_multi)
min.lambda <- which.min(PRS_multi$lambda)
fit_optimal_lambda <- glmnet(x = PPS_scored_prog_mat, y = y, family = "cox", lambda = PRS_multi$lambda[min.lambda])
...
for (i in 1:7){
  ...
  # if (i==2 | i==5 | i==7){
  # temp_mat <- as.data.frame(temp_mat) %>% subset(select=c(-Ethnicity6, -Ethnicity3, -Immigration2.5))
  # }
  set.seed(123)
  ...
  results_cox$calibration_intercept[i] <- logistic_calibration$intercept[1]
...
write_csv(results_cox, "~/Dropbox/Work/PPS/EU-GEI/results_cox_200824.csv")
...
write_csv(rf_results, "~/Dropbox/Work/PPS/EU-GEI/rf_results_190824.csv")
  ...
  for (outer_rep in 1:outerRepeats) {
    ...
    for (i in seq_along(outer_folds)) {
      ...
      for (inner_rep in 1:innerRepeats) {
        ...
        best_params <- results[which.max(inner_results$mean_cindex), ]
...
write_csv(rf_surv_results, "~/Dropbox/Work/PPS/EU-GEI/rf_surv_results_200824.csv")
...
dcaClin_PPS <- read_csv("~/Dropbox/Work/PPS/EU-GEI/dcaClin_PPS.csv")
dcaClin_PRS <- read_csv("~/Dropbox/Work/PPS/EU-GEI/dcaClin_PRS.csv")
dcaClin_PPSPRS <- read_csv("~/Dropbox/Work/PPS/EU-GEI/dcaClin_PPS+PRS.csv")
